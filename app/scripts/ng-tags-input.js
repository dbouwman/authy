...
function makeObjectArray(array, key) {
    console.log('MAKEOBJARRY key:' + key + ' Array:' + JSON.stringify(array));
    array = array || [];
...
tagsInput.directive('tagsInput', ["$timeout","$document","tagsInputConfig", "$log", function($timeout, $document, tagsInputConfig, $log) {
    function TagList(options, events) {
        ...
        self.add = function(tagText) {
            ...
            if (tagIsValid(tagText)) {
                self.items.push(tagText);
                events.trigger('tag-added', { $tag: tagText });
            }
            else {
                events.trigger('invalid-tag', { $tag: tagText });
            }
    ...
    return {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            ...
            scope.$watch('tags', function(value) {
                scope.tags = value;//makeObjectArray(value, options.displayProperty);
...
$templateCache.put('ngTagsInput/tags-input-back.html',
    ...
    "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-repeat=\"tag in tagList.items \" ng-class=\"{ selected: tag == tagList.selected }\"><span>{{tag}}</span> <a class=\"remove-button\" ng-click=\"tagList.remove($index)\">{{options.removeTagSymbol}}</a></li></ul><input class=\"input\" placeholder=\"{{options.placeholder}}\" tabindex=\"{{options.tabindex}}\" ng-model=\"newTag.text\" ng-change=\"newTagChange()\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ti-autosize=\"\"></div></div>"
  );
